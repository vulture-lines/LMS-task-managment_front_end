...
const ForumPostCard = ({ post, currentUser, onUpdate, onDelete, onRefresh }) => {
  ...
  const canEditOrDelete = currentUser._id === post.user._id;
  ...
  const handleShare = () => {
    ...
    navigator.clipboard.writeText(`${baseUrl}/student/forum/${post._id}`);
    setCopied(true);
...
const StudentForumPage = () => {
  ...
  const currentUser = { _id: "680373b6c9e849266316e9db", username: "student", role: "Student" }; // TODO: Replace with auth context
  ...
  const handlePostCreate = (newPost) => {
    // Since new posts require approval, they won't appear immediately
    ...
    <div className="container mx-auto py-6 max-w-4xl px-4">
      <div className="flex flex-col gap-6">
        <header className="flex flex-col gap-4">
          <h1 className="text-3xl font-bold">Student DevForum</h1>
        ...
        <div className="space-y-4">
          {filteredPosts.length === 0 ? (
            <p className="text-gray-500">No approved posts found.</p>
          ) : (
